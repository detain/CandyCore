...
**Rewritten 2026-09-08 post-P9 batch-2 (tip `ea81c377a`): PHASE 9 IN PROGRESS — 5 of 7 steps merged (S1 the promptGuidance seam; batch-2 S2 capability probes / S3 the Bash playbook fragment / S4 five tool man-pages / S7 HistorySanitizer), 58 of 65 steps done, NEW FLOOR 11,119 / 170,274 master-direct at `ea81c377a`; NEXT: P9.S5 staffing (GOLDEN-MOVING), then P9.S6 record-and-close, then the Phase-9 close review. The narration in sections 3-6 and the history blocks below are prior-state context.**

**Current state (updated 2026-09-08 after the P9 batch-2 merges S2/S3/S4/S7, tip `ea81c377a`): PHASE 9 IN PROGRESS — 5 of 7 steps merged (S1 seam, S2 probes, S3 Bash fragment, S4 five man-pages, S7 HistorySanitizer); 58 of 65 steps done.** **NEW FLOOR 11,119 / 170,274 / 0F / 0E / Skipped 2 / EXIT 0 — master-direct, the full suite run at the S7 merge tip `ea81c377a` itself (cmp vs the S4-era master.xml +26t/+195a, fully attributed; no belt needed — the gate is on master).** **NEXT: P9.S5 staffing — GOLDEN-MOVING (the affordance clauses + the budget statement + the R-1 `<git_commits>` cross-reference fold-in decision; a golden move is licensed ONLY at S5), then P9.S6 record-and-close, then the Phase-9 close review — FIVE worktrees S1..S4+S7 reaped after it.** GOLDEN ABSOLUTES UNCHANGED (system `a5c5a14ca2e3ad891933ac7aefccc6af`/7,732 B + agent `ef0326dd38535aaa2f1d715919bff26e`/1,060 B); seam census 22/134; TreeWideGuardRoster 17; the plan step-heading count re-derived this pass `grep -c '^### P[0-9]' prompt_plan.md` == 65. Per-step briefs live in `prompt_kit/briefs/` (committed working set - cite these paths, never a /tmp scratchpad). The §0 STANDING ORDER persists: run the plan to completion without pausing at phase boundaries; the re-derived push count lives in §8 item (2) - never trust a number in this file.**
...
Phases 0-7 are closed, and **PHASE 8 IS NOW CLOSED (2026-09-08)**: 5 of 5 steps merged - P8.S1 `e7de127d5` the
six-facet exchange records, P8.S2 `ce8530659` the anti-forgery + verbatim security constraints in the compaction
prompt, P8.S3 `1eef96e9a` the recursive prior-summary merge, P8.S4 `996ed0556` the 2,000-char head tool-output
bound, P8.S5 `c4c9b3b01` the compaction circuit breaker + E31/E32 - plus the cycle-1 CLOSE REVIEW (fresh tester;
verdict PHASE 8 MAY CLOSE, 0 BLOCKER / 0 MAJOR / 1 MINOR / 4 NIT) whose two folds (Q1 truthful carry prose, Q2
cap-notice tail byte-pin) landed in `b417dbbb7`. Numbered state, as this close-bookkeeping batch writes it:

1. **Master tip chain**: `c4c9b3b01` (P8.S5 merge) -> `9dc4e30f2` (plan annotate) -> `86c6f2e84` (resume) ->
   `b417dbbb7` (close-review folds) -> `0b11ede61` (worklog CLOSE entry) -> `1ad55d505` (plan CLOSED annotation)
   -> this resume commit LAST. Re-derive with `git log --oneline`; trust no number in this file.
2. **Unpushed**: `git rev-list --count origin/master..master` measured **36** at write time (33 at the P8.S5
   batch close + fold `b417dbbb7` + worklog `0b11ede61` + plan `1ad55d505`); **37** the moment this commit lands.
3. **Worktrees**: NONE live besides `/home/sites/sugarcraft` - all five P8 step worktrees removed after the close
   review, each HEAD proven ancestor-of-master first; **no `prompt/*` branches remain** (`git branch --list
   'prompt/*'` empty). Proof: `/tmp/opencode/P8-CLOSE/reap.txt`.
4. **53 of 65 steps done**; heading count re-derived at close with `grep -c '^### P[0-9]' prompt_plan.md` == 65.
5. **FLOOR: Tests 11,055 / Assertions 169,622 / 0F / 0E / Skipped 2 / EXIT 0, DIRECTLY gated at master
   `b417dbbb7`** (full-suite re-run on the committed master tip - the first master-direct gate figure since the
   belt era began, no belt needed). Prior floor 11,055/169,621 gated `f55114842`, superseded by the fold +1
   assertion.
6. **Goldens UNMOVED** (system `a5c5a14ca2e3ad891933ac7aefccc6af`/7,732 B + agent
   `ef0326dd38535aaa2f1d715919bff26e`/1,060 B); transcript seam census 20 and TreeWideGuardRoster 17/1127
   unchanged.
7. **Awaiting user decision: SIX items** - the five standing (see §8, carried verbatim) + NEW item (6): the Q6
   backlog E31-shape annotation (crush-lane file `crush_code_hardening_backlog.md:1760-1790` - mark E31
   CLOSED-with-variant-shape; the `compactNow` guidance is proven UNSOUND for parked routes, argument at
   Chat.php:9520-9540; goal met via the null + by-ref `$capNotice`, pinned ACMS :1572). Standing rule: no
   crush-lane touch without the user.
8. **Scheduled-unslotted steps reminder**: P7.S5b (deferred notice channel) + P6.S5b (BLOCKED on user item (4))
   + the NEW post-Phase-10 pair from the close review - E38 rider-source fix (ContextCompactor.php) and the
   PromptFence roster-widen 7->8 with the R-C wording re-ruling (the characterization pin at
   CompactModelSummaryTest:1571 reddens when it lands; that red IS the adjudication trigger).
9. **LESSON carried**: gate/reviewer claims are VOID without the junit artifact on disk - verify the tip.xml
   totals line before believing any GATE PASS.

Phases 9-11 remain (Phase 9 = Tool descriptions as prompt, plan `### P9.S1`..`### P9.S7`). Things a fresh agent
...
```
Phase:            Phase 9, 5 of 7 steps merged (S1 `731db08e1` + batch-2 S2 `9dd05ae06` / S3 `e9a933f4e` / S4 `f1f622e56` / S7 `ea81c377a`); Phase 8 CLOSED unchanged (5 of 5 + cycle-1 CLOSE REVIEW PASSED, folds b417dbbb7); Phases 0-8 = 8 of 12 CLOSED
Next step:        **P9.S5** — rebuild the base system-prompt fragments; GOLDEN-MOVING (the affordance clauses + the budget statement + the R-1 `<git_commits>` cross-reference fold-in decision; a golden move is licensed ONLY here, plan :2936+). Then **P9.S6** record-and-close (`src/Tui/Components/`), then the **Phase-9 close review** over S1..S7 together, then reap the five live worktrees. RE-DERIVE S5's scope line with grep at staffing.
                  STANDING NOTES: P6.S5b's §1.10 hold is LIFTED by the standing order (reaffirmed 2026-09-08) - its rule-body-transport question remains a standing user-decision item (§8 `Awaiting user decision:` item (4)), so it stays off the normal brief/build/schedule queue until answered. A §5 re-check is owed before P6.S5b AND before Phase 8 (`Chat.php` + `ContextCompactor.php`); the `EngineBackend.php`/`Cli/Bootstrap.php` rows were RE-CHECKED at P7.S3 provisioning and CONSUMED by merge `b289eaf44` (zero foreign commits through its window). The PHASE 6 CLOSE REVIEW is DONE (passed cycle 1); its whole-phase lens found the trigger union still UNWIRED, the `paths:`-scoped rule still rendering globally (the §18 row S5b owns), the `harness-injected` tag still defang-only with no emitter, and P6.S5b correctly UNBUILT (`src/Context/RulePathNudge.php` absent) - all PASS as combined-state facts, none a Phase-6 defect. The 'triggers-built-not-applied' thread (open-follow-up (8)) is discharged for `findForPrompt`/`matchesPrompt` BY THE P7.S4 DECISION ITSELF (dormant-with-number) - the CURATED-frontmatter-keywords matcher design remains an unscheduled feature idea recorded in the P7.S4 premise.
                  STANDING NOTES (2026-09-08): User directive 2026-09-08: run plan-sanctioned concurrency up to 5 agents (Phase 8 serial by plan :2820; Phase 9 batch-2 four-way :2962-2969); pipeline read-only scouts during serial phases; verify brief premises against shipped semantics (law 4m).
Steps done:       58 of 65. (batch-2: P9.S2 `9dd05ae06` #55, P9.S3 `e9a933f4e` #56, P9.S4 `f1f622e56` #57, P9.S7 `ea81c377a` #58; P9.S1 was #54. Heading count RE-DERIVED this pass: `grep -c '^### P[0-9]' prompt_plan.md` == 65.)
Phases done:      8 of 12 CLOSED (P0-P6 as before - Phase 6 closed via its cycle-1 close review; Phase 7 closed via its 2026-09-08 close-out review CLEAN-WITH-BOOKKEEPING, six of six merged; Phase 8 closed via the 2026-09-08 cycle-1 close review, 0 BLOCKER / 0 MAJOR / 1 MINOR / 4 NIT, folds in b417dbbb7, five step worktrees + branches reaped; P7.S5b rides along as plan prose, not a counted step)
Last commit:      re-derive with git log - chain: b417dbbb7 (close folds) -> 0b11ede61 (worklog CLOSE entry) -> 1ad55d505 (plan CLOSED annotation) -> this resume commit lands LAST
Baseline:         Tests: 10351, Assertions: 160648, Skipped: 1  (from P0.S1, NEVER edited)
Latest suite:     **NEW FLOOR - Tests 11,119 / Assertions 170,274 / 0F / 0E / Skipped 2 / EXIT 0**, master-direct (the full suite run directly at the P9.S7 merge tip `ea81c377a`; artifact /tmp/opencode/P9.S7-merge/master.xml totals verified — the belt is unnecessary because the gate IS on master). cmp vs the S4-era master.xml = +26t/+195a, fully attributed: HistorySanitization +25t/+87a, BinSugarcrush +1t/+6a (new-src-file enrollment), Glob +37 (35 DocumentParagraphs + 2 file-reads), EnvRoster +9, SymCite +6, StderrCensus +16, DocParas +4, TreeWide +2, walk-wave 10 classes +28; every behavioral suite delta 0, goldens FROZEN ×3 merges. Batch-2 step floors (each master-direct at its merge): S2 11,074/169,958 · S3 11,081/170,033 · S4 11,093/170,079 · S7 11,119/170,274. Seam census 22/134; TreeWideGuardRoster 17. Full record: worklog P9.S7 entry.
In-flight batch:  none — Phase-9 batch 2 CLOSED (S2/S3/S4/S7 all merged + master-direct gated); no builders running now.
Live worktrees:   main + the FIVE Phase-9 step worktrees — /home/sites/prompt-step-P9.S1 @ `2c6583561`, -P9.S2, -P9.S3, -P9.S4, -P9.S7 (branches prompt/P9.S1..S4, prompt/P9.S7). All alive for the Phase-9 close review, reaped after it per the P8 doctrine (HEAD-proven ancestor-of-master first, `git worktree remove --force` for untracked cache/vendor only, branches with plain `git branch -d`, no -D). `git worktree list` = exactly six (main + 5).
Unpushed:         **62** — MEASURE, do not copy: re-derived by formula, 59 at this bookkeeping's start +3 for the batch (worklog `e567c3329` + plan `ba1e106c3` + this resume commit) = 62 the moment this resume lands (61 in between). Re-derive every rewrite with `git -C /home/sites/sugarcraft rev-list --count origin/master..master`; if yours differs, origin/master moved and yours is the truth.
Blocked on:       none; standing orders run plan to completion
